...
def test_p01_standard_cases(reverse_words):
    assert reverse_words("the sky is blue") == "blue is sky the"
    assert reverse_words("Hello world") == "world Hello"
    assert reverse_words("I AM SO BORED!") == "BORED! SO AM I"

def test_p01_whitespace_preservation(reverse_words):
    assert reverse_words("  hello world  ") == "  world hello  "
    assert reverse_words("a   b  c") == "c   b  a"
    assert reverse_words("   ilove coding") == "   coding ilove"

def test_p01_edge_cases(reverse_words):
...
def test_p02_standard_cases(first_unique_char):
    assert first_unique_char("helloworld") == 0
    assert first_unique_char("helloworldhellosun") == 5

def test_p02_no_unique_char(first_unique_char):
    assert first_unique_char("aabb") == -1
    assert first_unique_char("cc") == -1

def test_p02_single_and_last_char(first_unique_char):
...
def test_p03_found_elements(search_rotated_array):
...
def test_p03_missing_elements(search_rotated_array):
...
def test_p04_heavy_multi_day_discount(min_cost_tickets):
...
def test_p04_very_expensive_multi_day_passes(min_cost_tickets):
...
def test_p05_fifo_and_size(BoundedBlockingQueue):
...
def test_p05_blocking_dequeue(BoundedBlockingQueue):
...
def test_p05_blocking_enqueue(BoundedBlockingQueue):
